...
import { getShippingFee, getFreeShippingThreshold } from '../../lib/shipping'
...
export default function CheckoutPage() {
  ...
  const orderTotal = Math.max(subtotal - discountAmount, 0) + shippingFee
  ...
  const applyCoupon = async () => {
    ...
    setCouponApplied({ code: couponInput.trim().toUpperCase(), discountAmount: data.discountAmount })
  }
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      items: items.map((i) => ({ id: i.id, name: i.name, price: i.price, qty: i.qty, type: i.type || 'product' })),
    ...
    <div className="max-w-container mx-auto px-4 md:px-8 py-16">
      ...
      <form onSubmit={handleSubmit} className="grid md:grid-cols-3 gap-10">
        <div className="md:col-span-2 flex flex-col gap-10">
          ...
          <section>
            ...
            {savedAddresses.length > 0 && (
              <div className="mb-4">
                <label className="label-field">Endereços salvos</label>
                <select
                ...
                >
                  ...
                  {savedAddresses.map((a) => (
                    <option key={a.id} value={a.id}>{a.label || `${a.rua}, ${a.numero}`}</option>
                  ))}
                </select>
              </div>
            )}
            ...
            {user && (
              <label className="flex items-center gap-2 text-sm text-muted mt-4 cursor-pointer">
        ...
        <div className="border border-line rounded-sm p-6 h-fit flex flex-col gap-4">
          ...
          <div>
            <div className="flex gap-2">
              ...
              {couponApplied ? (
                ...
                <button type="button" onClick={applyCoupon} disabled={couponChecking} className="btn-outline text-xs whitespace-nowrap">
                  {couponChecking ? 'Verificando…' : 'Aplicar'}
                </button>
              )}
            </div>
            {couponError && <p className="text-xs text-stamp mt-1">{couponError}</p>}
            {couponApplied && <p className="text-xs text-teal mt-1">Cupom {couponApplied.code} aplicado!</p>}
          </div>
          <div className="flight-divider" />
          ...
          <div className="flex justify-between text-sm text-muted">
            <span>Frete</span>
            <span className="font-mono">{shippingFee > 0 ? formatBRL(shippingFee) : 'Grátis'}</span>
          </div>
          {shippingFee > 0 && freeShippingThreshold > 0 && (
            <p className="text-xs text-muted -mt-2">
              Frete grátis a partir de {formatBRL(freeShippingThreshold)} em compras.
            </p>
          )}
          <div className="flight-divider" />
          <div className="flex justify-between font-semibold text-lg">
            <span>Total</span>
            <span className="font-mono">{formatBRL(orderTotal)}</span>
          </div>
