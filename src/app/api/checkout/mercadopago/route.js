...
export async function POST(request) {
  try {
    ...
    // Um único item com o total já calculado no servidor (produtos + kits -
    // desconto do cupom + frete) — evita qualquer divergência entre o que foi
    // cobrado no Mercado Pago e o que ficou salvo no pedido.
    const preferenceItems = [{
      title: `Pedido Dos States ${orderDisplayCode(order.id)}`,
      quantity: 1,
      unit_price: total,
      currency_id: 'BRL',
    }]
